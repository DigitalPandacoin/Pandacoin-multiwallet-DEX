...
namespace atomic_dex
{
    ...
    qt_orderbook_wrapper::refresh_orderbook_model_data(kdf::orderbook_result_rpc answer)
    {
        ...
        if (sw.elapsed().count() > 0.01) { SPDLOG_DEBUG("Time elapsed in qt_orderbook_wrapper::refresh_orderbook_model_data with bids/asks size {}/{}: {}", answer.bids.size(), answer.asks.size(), duration_cast<milliseconds>(sw.elapsed())); }
    }
