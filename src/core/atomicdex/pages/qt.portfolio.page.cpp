...
namespace atomic_dex
{
    ...
    portfolio_page::set_current_balance_fiat_all(QString current_fiat_all_balance)
    {
        if (this->m_current_balance_all != current_fiat_all_balance)
        {
            ...
            SPDLOG_DEBUG("Time elapsed in portfolio_page::set_current_balance_fiat_all to change from {} to {}: {}", m_current_balance_all.toStdString(), current_fiat_all_balance.toStdString(), duration_cast<milliseconds>(sw.elapsed()));
        }
    }
